...
class model(nn.Module):
    def __init__(self):
        ...
        # encoder
        self.pretrained_model = resnet152(pretrained=True)
        ...
        self.mlp1 = nn.Linear(2048, 4096)
        self.mlp2 = nn.Linear(4096, 2048)
        self.upsample = nn.Upsample(16)

        # decoder
        self.dec = Decoder(2, 2048, 2, activ='relu', pad_type='reflect')
    ...
    def generate_attention(self, x):
        ...
        attention_x = self.upsample(torch.sigmoid(self.mlp2(
            torch.tanh(self.mlp1(feature_x.view(-1, 2048))))).view(-1, 2048, 1, 1))
...
class Decoder(nn.Module):
    def __init__(self, n_res, dim, output_dim, activ='relu', pad_type='zero'):
        ...
        for i in range(2):
            self.model += [Conv2dBlock(dim, dim // 2, 3, 1, 1,
                                       norm='bn', activation=activ, pad_type='reflect')]
            dim //= 2
