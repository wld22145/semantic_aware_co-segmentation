...
class model(nn.Module):
    def __init__(self):
        ...
        self.spatial_avg_pool = nn.AdaptiveAvgPool3d((1, 16, 16))
    ...
    def forward(self, x, y):
        ...
        vgg_x, vgg_y, vgg_x_weight, vgg_y_weight,vgg_x_spatial,vgg_y_spatial = self.encode(x, y)
        images_recon_x, images_recon_y = self.decode(
            vgg_x, vgg_y, vgg_x_weight, vgg_y_weight,vgg_x_spatial,vgg_y_spatial)
    ...
    def generate_attention(self, x):
        ...
        attention_x = self.upsample(torch.sigmoid(self.mlp2(
            torch.tanh(self.mlp1(feature_x.view(-1, 512))))).view(-1, 512, 1, 1))
        spatial_attention_x = torch.tanh(self.spatial_avg_pool(vgg_x))
        return vgg_x,attention_x,spatial_attention_x

    def encode(self, x, y):
        # encode an image to its content
        vgg_x,attention_x,spatial_x = self.generate_attention(x)
        vgg_y,attention_y,spatial_y = self.generate_attention(y)

        return vgg_x, vgg_y, attention_x, attention_y,spatial_x,spatial_y


    def decode(self, vgg_x, vgg_y, attention_x, attention_y,spatial_x,spatial_y):
        attention_avg = attention_x + attention_y
        # decode content to an image
        mask_x = self.dec(attention_avg * vgg_x)
        mask_y = self.dec(attention_avg * vgg_y)
