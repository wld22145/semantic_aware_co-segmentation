...
class model(nn.Module):
    ...
    def encode(self, x, y):
        ...
        vgg_x = self.generate_attention(x)
        vgg_y = self.generate_attention(y)

        return vgg_x, vgg_y


    def decode(self, vgg_x, vgg_y):
        # decode content to an image
        mask_x = self.dec(vgg_x)
        mask_y = self.dec(vgg_y)
